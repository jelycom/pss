...
<script type="text/javascript">
	$(function() {
		var lastSel;
		$('#jxctable').jxcGrid({
			colNames:['rid','单据编号','收货日期','总金额','未付金额','结算金额','备注','收货单号'],
			colModel:[
...
<<<<<<< .mine
				,	{name:'amount',width:100,align:'right',editable:false}
				,	{name:'arap',width:100,align:'right',editable:false}
/* 				,	{name:'paidArap',width:100,align:'right',editable:false}
				,	{name:'needPay',width:100,align:'right',editable:false} */
				,	{name:'currentPay',width:100,editrules:{number:true}}
=======
...
>>>>>>> .r38
	        	...
	        	if (!$(this).parents(".ui-dialog").is("[id^=view]")){
		        	var editRowSet={keys:false,									//行编辑时的参数
				        			oneditfunc:function(rowid){
			    		        		...
			    		        		$sum.change(function(){
			    		        			var val=Number($sum.val()).toFixed(2);
			    		        			$sum.val(val);
		...
		var gridoptions={
			url:'accountOut_listjson.action',
			caption:'采购付款单管理',
		...
		$('.definedForm').bind('dialogopen',function(event,ui){
			var $extendbuttons = $("<div class='extend-buttons' style='float:left;'>"+
									"<button onclick='selectBill(\"productPurchase\")'>选择收货单</button>"+
									"<button id='viewBillButton'>查看收货单</button>"+
...
<body>
	...
	<div class="definedForm">
		<form id="mainform" action="accountOut_save.action" method="POST">
			<table class="out_table">
				<tr>
	            	...
	            	<td>
	            		<input type="text" id="item" name="item" readonly="readonly" url="accountOut_getItem.action" />
	            	</td>
	            </tr>
	            <tr>
	            ...
	            <tr>
					...
					<td><label for="amount">付款金额</label></td>
