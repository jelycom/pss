	...
	$(function() {
		$("#jxctable").jxcGrid({
			...
			colModel:[
				...
				,	{name:'amount',editrules:{number:true},editoptions:{readonly:true},total:true}
		...
		var gridoptions={
			url:'productOrderBillPurchase_listjson.action',
			caption:'采购订单管理',
			colNames:['','录单日期','交货日期','单据编号', '供货单位','联系人','经手人', '收货仓库','总金额','状态'],
			colModel:[
				...
				,	{name:'contactor.name',width:100,search:false}
				...
				,	{name:'state.msg',formatter:'statf',width:60}
			]
		};//主表的参数
		var naviPrm={
				parameter:{editfunc:editGrid,addfunc:addGrid,viewfunc:viewGrid},		//自定义的增删改查方法
				del:{url:'productOrderBillPurchase_delete.action'}
			};
		var formatter=formatterFunc();
		$('#list').selfGrid(gridoptions,naviPrm,formatter);
		
		$('#businessUnitName,#employeeName,#warehouseName').completeGrid([{},{reload:false,createInput:false}]);
//		$('#invoiceName').completeGrid([{},{createInput:false}]);
		$(".definedForm").bind("dialogopen",function(event,ui){
			var $extendbuttons = $("<div class='extend-buttons' style='float:left;'>"+
									"<button onclick='selectPlan(\"productPlanPurchase\")'>选择计划单</button>"+
									"</div>")
...
<body>
	...
	<div class="definedForm">
		<form id="mainform" action="productOrderBillPurchase_save.action" method="POST">
			<div class="add_del">
			...
			<table class="out_table" id="form_header">
				<tr>
	            	...
	            	<td>
	            		<input type="text" id="item" name="item" readonly="readonly" url="productOrderBillPurchase_getItem.action" />
	            	</td>
	            </tr>
	            <tr>
	            	<td><label for="businessUnitName">供货单位</label></td>
	            	<td>
	            	...
	            	<td><label for="warehouseName">收货仓库</label></td>
					<td>
	        ...
	        <table class="out_table">
	        	<tr>
	                ...
	                <td><label for="paid">已付订金</label></td>
