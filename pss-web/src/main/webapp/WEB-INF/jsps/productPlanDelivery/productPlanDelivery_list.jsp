	...
	$(function() {
		$("#jxctable").jxcGrid({
			...
			colModel:[
				...
				,	{name:'quantity',width:120,align:'right',editrules:{number:true}}
				,	{name:'memo'}
			]
		});//进销存表格的参数
		var gridoptions={
				url:'productPlanDelivery_listjson.action',
				...
				caption:'销售计划单管理',
				viewurl:'productPlanDelivery_viewData.action'
			};
		var naviPrm={
			parameter:{editfunc:editGrid,addfunc:addGrid,viewfunc:viewGrid},
			del:{url:"productPlanDelivery_delete.action"}
		};
		var formatter=formatterFunc();
		$("#list").selfGrid(gridoptions,naviPrm,formatter);
		$("#planEmployeeName,#executeEmployeeName").completeGrid([{},{createInput:false}]);		//计划人员自动完成参数
	});	
...
<body>
	...
	<div class="definedForm">
		<form id="mainform" action="productPlanDelivery_save.action" method="post">
			<div class="add_del">
			...
			<table class="out_table" id="form_header">
				<tr>
	            	...
	            	<td>
	            		<input type="text" id="item" name="item" readonly="readonly" url="productPlanDelivery_getItem.action" />
	            	</td>
	            </tr>
	            <tr>
	            ...
	            <tr>
					...
					<td>
						<input id="executeEmployeeId" name="executeEmployee.id" url="employee_listjson.action" type="hidden" required=true title="计划执行人员" />
						<input id="executeEmployeeName" name="executeEmployeeName" type="text" />
					</td>
					<td><label for="remark">备注</label></td>
					<td><input id="remark" name="" type="text" /></td>
	            </tr>
	        </table>
	        <!-- 表头结束 -->
	        <!-- 数据部分 -->
