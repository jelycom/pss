...
<script type="text/javascript">
	$(function() {
	        	...
	        	if (!$(this).parents(".ui-dialog").is("[id^=view]")){
		        	...
		        	if (id!=lastSel&&lastSel){				//已有选中行时，保存已选中行
		        		if(!$("#"+lastSel+" :text:first",$jxcGrid).val()){
		        			$jxcGrid.restoreRow(lastSel);	//项目为空时，恢复成空行
		...
		var gridoptions={
			url:'accountOtherIn_listjson.action',
			caption:'其他收入管理',
			colNames:['','录单日期','单据编号','付款单位','经手人', '收款帐户','收款金额','优惠','状态'],
			colModel:[
		...
		var naviPrm={
				...
				del:{url:'accountOtherIn_delete.action'}
			};
	...
	function bursaryCompSet(){
		return	[{
			...
			postData:{'filters':'{"groupOp":"AND","rules":['+
			                       	'{"field":"lft","op":"gt","data":"29"},'+
			                       	'{"field":"rgt","op":"lt","data":"36"}]}'},
...
<body>
	...
	<div class="definedForm">
		<form id="mainform" action="accountOtherIn_save.action" method="POST">
			<table class="out_table">
				<tr>
	            	...
	            	<td>
	            		<input type="text" id="item" name="item" readonly="readonly" url="accountOtherIn_getItem.action" />
	            	</td>
	            </tr>
	            <tr>
