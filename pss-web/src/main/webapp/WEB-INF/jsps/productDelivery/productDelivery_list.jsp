	...
	$(function() {
		$('#jxctable').jxcGrid({
			...
			colModel:[
				...
				,	{name:'orderBillID',editable:false,hidden:true,gridview:false}
			]
		});//进销存表格的参数
		
		var gridoptions={
			url:'productDelivery_listjson.action',
			caption:'销售出货单管理',
			colNames:['','录单日期','单据编号','客户单位','联系人','经手人', '发货仓库','总金额','状态'],
			colModel:[
		...
		var naviPrm={
				...
				del:{url:'productDelivery_delete.action'}
			};
		...
		$('.definedForm').bind('dialogopen',function(event,ui){
			var $extendbuttons=$("<div class='extend-buttons' style='float:left;'>"+
								 ...
								 "<button onclick='selectOrder(\"productOrderBillDelivery\")'>选择订单</button>"+
								 "</div>")
	...
	function subData(condition,$formDiv,options){
		...
		$('#jxctable tr[id]').each(function(i){
	  		...
	  		if(condition != 'add'&&rowData['productId']){
	  			$details.append("<tr class='details'><td><input type='hidden' name='details["+i+"].product.id' value='"+rowData['productId']+"'/></td>"+
...
<body>
	...
	<div class="definedForm">
		<form id="mainform" action="productDelivery_save.action" method="POST">
			<div class="add_del">
			...
			<table class="out_table">
				<tr>
	            	...
	            	<td>
	            		<input type="text" id="item" name="item" readonly="readonly" url="productDelivery_getItem.action" />
	            	</td>
	            </tr>
	            <tr>
