...
<script type="text/javascript">
	$(function() {
		$('#jxctable').jxcGrid({
			colNames:['rid','商品号','商品名称','规格','颜色','单位','数量','单价','金额','含税单价','税金','价税合计','备注','计划单号','订单号'],
			colModel:[
				...
				,	{name:'orderBillID',editable:false}
			]
		});//进销存表格的参数
		
		var gridoptions={
			url:'productPurchase_listjson.action',
			caption:'采购收货单管理',
			colNames:['','录单日期','单据编号','供货单位','联系人','经手人', '收货仓库','总金额','状态'],
			colModel:[
				...
				,	{name:'contactor.name',hidden:true,search:false}
		...
		var naviPrm={
				...
				del:{url:'productPurchase_delete.action'}
			};
		...
		$('.definedForm').bind('dialogopen',function(event,ui){
			var $extendbuttons = $("<div class='extend-buttons' style='float:left;'>"+
									...
									"<button onclick='selectOrder(\"productOrderBillPurchase\")'>选择订单</button>"+
									"</div>")
	  		...
	  		if(condition!='add'&&rowData['productId']){
	  			$details.append("<tr class='details'><td><input type='hidden' name='details["+i+"].id' value='"+rowData['rid']+"'/></td>"+
		  				"<td><input type='hidden' name='details["+i+"].product.id' value='"+rowData['productId']+"'/></td>"+
		   				...
		   				"<td><input type='hidden' name='details["+i+"].orderBillMaster.id' value='"+rowData['orderBillID']+"'/></td>"+
	   				"</tr>");
	  	 	}
	 	});
...
<body>
	...
	<div class="definedForm">
		<form id="mainform" action="productPurchase_save.action" method="POST">
			<div class="add_del">
	        ...
	        <table class="out_table">
	        	<tr>
	        		<td><label for="advance">此单预付</label></td>
	                ...
	                <td><label for="paid">实付金额</label></td>
