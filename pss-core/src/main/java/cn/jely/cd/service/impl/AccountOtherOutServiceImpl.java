/*
 * 捷利商业进销存管理系统
 * @(#)AccountOtherOutServiceImpl.java
...
import cn.jely.cd.dao.IAccountOtherOutDao;
 ...
 */
public class AccountOtherOutServiceImpl extends BillStateServiceImpl<AccountOtherOutMaster> implements IAccountOtherOutService {
	private IAccountOtherOutDao accountOtherOutDao;
	private IFundAccountDao fundAccountDao;

	public void setAccountOtherOutDao(IAccountOtherOutDao accountOtherOutDao) {
		this.accountOtherOutDao = accountOtherOutDao;
	}
	...
	@Override
	protected AccountOtherOutMaster getOldDomain(AccountOtherOutMaster master) {
		return getById(master.getId());
	}

	@Override
	protected void prepareModel(AccountOtherOutMaster master) {
		if(master==null){
	 ...
	 * @param master void
	 */
	private BigDecimal checkDetails(AccountOtherOutMaster master) {
	...
	@Override
	protected void performChange(AccountOtherOutMaster master) {
		fundAccountDao.subCurrent(master.getFundAccount().getId(), master.getAmount());
	}

	@Override
	protected IBaseDao<AccountOtherOutMaster> getBaseDao() {
		return accountOtherOutDao;
	}
