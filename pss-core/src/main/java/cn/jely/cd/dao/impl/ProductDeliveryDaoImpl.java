...
/**
 * @ClassName:ProductDeliveryMasterDaoImpl
 * @Description:DaoImpl
 * @author
...
public class ProductDeliveryDaoImpl extends ProductDeliveryDaoAbstractImpl<ProductDeliveryMaster> implements IProductDeliveryDao {
	...
	public List<ProductCommonMaster> findUnFinishedWithReturn(ObjectQuery objectQuery) {
		...
		queryGroup.getRules().add(new QueryRule("paidArap is not null and type(o) in (ProductDeliveryMaster,ProductDeliveryReturnMaster) and state", FieldOperation.in, StateManager.getUnCompleteStates()));
//
//		objectQuery.setQueryClass(ProductCommonMaster.class);
//		if(StringUtils.isNotBlank(objectQuery.getWhere())){
//			objectQuery.addWhere("and paidArap is not null and type(o) in (ProductDeliveryMaster,ProductDeliveryReturnMaster) and state in (:states)", "states", StateManager.getUnCompleteStates());
//		}else{
//			objectQuery.addWhere("paidArap is not null and type(o) in (ProductDeliveryMaster,ProductDeliveryReturnMaster) and state in (:states)", "states", StateManager.getUnCompleteStates());
//		}
