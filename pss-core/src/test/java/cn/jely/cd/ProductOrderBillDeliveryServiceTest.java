 ...
 *
 */
public class ProductOrderBillDeliveryServiceTest extends BaseServiceTest{
	...
	public void testGenerateItem(){
		String item=((ItemGenAble)productOrderBillDeliveryService).generateItem(new Date());
	...
	public void testSave() {
		...
		List<ProductPlanDeliveryMaster> allPlans=productPlanDeliveryService.findAllUnFinished();
		List<Warehouse> allhouses=warehouseService.getAll();
		ActionResource resource=actionResourceService.findQueryObject(new ObjectQuery().addWhere("linkAddress=:address", "address", "cn.jely.cd.web.ProductOrderBillDeliveryAction.list"));
...
//		State state=states.get(0);
		for(int i=0;i<5;i++){
			...
			ProductOrderBillDeliveryMaster master=new ProductOrderBillDeliveryMaster(companyUnit,warehouse,executeEmployee,account);
			master.setItem(((ItemGenAble)productOrderBillDeliveryService).generateItem(new Date()));
			master.setProductPlans(allPlans);
...
//			master.setState(state);
		List<ProductOrderBillDetail> details = new ArrayList<ProductOrderBillDetail>();
		for(ProductPlanDeliveryMaster plan:subPlans){
...
//				detail.setQuantity(plandetail.getPlanQuantity());
	...
	public void testUpdate() {
		try {
			ProductOrderBillDeliveryMaster master=(ProductOrderBillDeliveryMaster) getRandomObject(productOrderBillDeliveryService.getAll());
			Long oldId=master.getId();
			Integer oldSize=master.getDetails().size();
//		productOrderBillDeliveryService.changeState(master, ProductOrderBillMaster.DISCARD); 
	...
	public void testUpdateState() {
		Pager<ProductOrderBillDeliveryMaster> pager=productOrderBillDeliveryService.findPager(null);
		ProductOrderBillDeliveryMaster master=(ProductOrderBillDeliveryMaster) getRandomObject(productOrderBillDeliveryService.getAll());
